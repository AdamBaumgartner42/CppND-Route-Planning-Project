...
RouteModel::Node *RoutePlanner::NextNode() {
    ...
    // sort open_list to have the smallest "h+g" sum at the end
...
std::vector<RouteModel::Node> RoutePlanner::ConstructFinalPath(RouteModel::Node *current_node) {
    ...
    // Adds the start node && reverse the order
    path_found.push_back(*current_node); 
...
void RoutePlanner::AStarSearch() {
    ...
    while(!this->open_list.empty()){
        // inside the loop you shall first pick the next node
        current_node = NextNode();
        // Check if it is the indended node via checking against the end node
        if(current_node == this->end_node){
            // If yes then construct the final path and break
            m_Model.path = ConstructFinalPath(current_node);
            break;
        } else {
            // If no, add another neighbor and iterate again
